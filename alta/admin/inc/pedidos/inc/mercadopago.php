<?php
include($_SERVER['DOCUMENT_ROOT'] . "/vendor/mercadopago/sdk/lib/mercadopago.php");
$config = new Clases\Config();
$config->set("id", 1);
$paymentsData = $config->viewPayment();
$mp = new MP ($paymentsData['data']['variable1'], $paymentsData['data']['variable2']);
$preference_data = array("items" => array(array("id" => $_SESSION["cod_pedido"], "title" => "COMPRA CÓDIGO N°:" . $_SESSION["cod_pedido"], "quantity" => 1, "currency_id" => "ARS", "unit_price" => $precio)), "payer" => array("name" => $_SESSION["usuarios-ecommerce"]["nombre"], "surname" => $_SESSION["usuarios-ecommerce"]["apellido"], "email" => $_SESSION["usuarios-ecommerce"]["email"]), "back_urls" => array("success" => URL . "/compra-finalizada.php?estado=2", "pending" => URL . "/compra-finalizada.php?estado=1", "failure" => URL . "/compra-finalizada.php?estado=0"), "notification_url" => URL . "/ipn.php", "external_reference" => $_SESSION["cod_pedido"], "auto_return" => "all", //"client_id" => $usuarioSesion["cod"],
