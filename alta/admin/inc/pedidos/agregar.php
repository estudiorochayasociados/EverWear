...
if (isset($_GET['usuario'])) {
    ...
    if (!empty($usuarioData['data'])) {
        ...
        $funciones->headerMove(URL . '/index.php?op=usuarios&accion=ver');
    }
} else {
    unset ($_SESSION['usuarios-ecommerce']);
    $funciones->headerMove(URL . '/index.php?op=usuarios&accion=ver&pedido=1');
}
...
if (!empty($usuarioData)) {
    $descuentoCheck = $descuento->descuentoCheck($carroData, $usuarioData);
} else {
    $descuentoCheck = $descuento->descuentoCheck($carroData);
}
...
<div class="mt-20 col-md-12">
    ...
    if (isset($_SESSION['usuarios-ecommerce'])) {
        ...
        <div class="alert alert-success" role="alert">
            ...
            Si desea eliminar este carrito para hacer uno nuevo con otro usuario, haga click <a href="<?= URLADMIN . '/index.php?op=pedidos&accion=agregar&reset=1' ?>">aquí</a>
        </div>
        <?php
    }
    if (!empty($error)) {
        ...
        <div class="col-md-5">
            ...
            <table class="table table-bordered table-condensed table-hover">
                ...
                if (isset($_GET["remover"])) {
                    ...
                    $funciones->headerMove(URL . "/index.php?op=pedidos&accion=agregar&usuario=" . $_SESSION['usuarios-ecommerce']['cod']);
                }
                $i = 0;
                $precio = 0;
            ...
            <div class="col-md-12 hidden-xs hidden-sm hidden-lg hidden-md">
                ...
                if (isset($_POST["btn_codigo"])) {
                    ...
                    if (!empty($usuarioData)) {
                        $descuento->refreshCart($carroData, $usuarioData);
                    } else {
                        $descuento->refreshCart($carroData);
                    }
                    $funciones->headerMove(CANONICAL);
                }
                ?>

                <?php if ($descuentoCheck['check'] == 1) { ?>
                ...
                <?php } else { ?>
                    <form method="post" class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <input type="submit" value="USAR" name="btn_codigo" class="btn btn-info"/>
                            </div>
                        </div>
                    </form>
                <?php } ?>
...
//                        }
                        ...
                        $detalleCompleto['envio']['nombre'] = $_SESSION["usuarios-ecommerce"]['nombre'];
                        $detalleCompleto['envio']['apellido'] = $_SESSION["usuarios-ecommerce"]['apellido'];
                        $detalleCompleto['envio']['email'] = $_SESSION["usuarios-ecommerce"]['email'];
                        $detalleCompleto['envio']['provincia'] = $_SESSION["usuarios-ecommerce"]['provincia'];
                        $detalleCompleto['envio']['localidad'] = $_SESSION["usuarios-ecommerce"]['localidad'];
                        $detalleCompleto['envio']['direccion'] = $_SESSION["usuarios-ecommerce"]['direccion'];
                        $detalleCompleto['envio']['telefono'] = $_SESSION["usuarios-ecommerce"]['telefono'];

                        $detalleCompleto['pago']['nombre'] = $_SESSION["usuarios-ecommerce"]['nombre'];
                        $detalleCompleto['pago']['apellido'] = $_SESSION["usuarios-ecommerce"]['apellido'];
                        $detalleCompleto['pago']['email'] = $_SESSION["usuarios-ecommerce"]['email'];
                        $detalleCompleto['pago']['dni'] = $_SESSION["usuarios-ecommerce"]['doc'];
                        $detalleCompleto['pago']['provincia'] = $_SESSION["usuarios-ecommerce"]['provincia'];
                        $detalleCompleto['pago']['localidad'] = $_SESSION["usuarios-ecommerce"]['localidad'];
                        $detalleCompleto['pago']['direccion'] = $_SESSION["usuarios-ecommerce"]['direccion'];
                        $detalleCompleto['pago']['telefono'] = $_SESSION["usuarios-ecommerce"]['telefono'];
                        if ($factura == 1) {
                    ...
                    <form method="post">
                        ...
                        if ($docGet == 1) {
                            ...
                            if (!empty($_SESSION['usuarios-ecommerce']['doc'])) {
                                ...
                                ?>
                                <label class=" mt-10 mb-10" style="font-size:16px">
                                    <input onclick="document.location.href='<?= URLADMIN . '/index.php?op=usuarios&accion=modificar&cod=' . $usuarioData['data']['cod'] . '&pedido=2'; ?>'"
...
<script>
    function addAttrComb(cod) {
        $.ajax({
            url: "<?=URLSITE?>/curl/atributes/view.php",
            type: "POST",
            data: {cod: cod},
