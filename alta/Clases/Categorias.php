...
class Categorias
{
    ...
    {
        $array = array();
        $sql = " SELECT `categorias`.`titulo`,`categorias`.`cod`,`categorias`.`id`, count(`" . $db . "`.`categoria`) as cantidad FROM `" . $db . "`,`categorias` WHERE `categoria` = `categorias`.`cod` GROUP BY categoria ORDER BY cantidad DESC ";
