...
class Imagenes
{
    ...
    {
        $url = dirname(__DIR__).'/archivos/img_productos/' . $variable . '.jpg';
        if (@getimagesize($url)) {
            $ruta = 'archivos/img_productos/' . $variable . '.jpg';
        } else {
    ...
    public function resizeImages($cod, $files, $path, $final_path)
    {
        foreach ($files['name'] as $f => $name) {
            $tmpFile = $files["tmp_name"][0];
            $nameFile = $files["name"][0];
            ...
            if ($extension != '') {
                ...
                if ($this->zebra->resize(0, 0, ZEBRA_IMAGE_NOT_BOXED)) {
                    unlink($path_);
                }
