...
class PublicFunction
{
    ...
    {
        $_SESSION["csrf-token"] = !isset($_SESSION["csrf-token"]) ? $this->tokenGenerate() : $this->tokenGet();
    }
    ...
    public function tokenUnset()
    {
        $_SESSION["csrf-token"] = $this->tokenGenerate();
    }
    ...
    public function cities($state)
    {
        ...
        $palabra = $state;
        $sql = "SELECT  distinct `_provincias`.`nombre`,`_localidades`.`nombre` FROM  `_localidades` , `_provincias` WHERE  `_localidades`.`provincia_id` =  `_provincias`.`id` AND `_provincias`.`nombre`  LIKE '%$palabra%' AND `_localidades`.`nombre` != '' ORDER BY `_localidades`.`nombre`";
        $response = $con->sqlReturn($sql);

        $cities = [];
        while ($row = mysqli_fetch_assoc($response)) {
            $cities[] = strtoupper($row["nombre"]);
        }
        return $cities;
    }
    ...
    public function provincias()
    {
        ...
        while ($row = mysqli_fetch_assoc($provincias)) {
            echo '<option value="' . mb_strtoupper($row['nombre']) . '">' . mb_strtoupper($row['nombre']) . '</option>';
        }
    }
    ...
    function curlSelf($method, $url, $data)
    {
        $curl = curl_init();
