...
class Productos
{
    ...
    public function view(array $product = [], $full = true)
    {
        $array = '';
        if (empty($product)) {
            $sql = "SELECT * FROM `productos` WHERE  cod = {$this->cod} LIMIT 1";
            $row = mysqli_fetch_assoc($this->con->sqlReturn($sql));
        } else {
            $row = $product;
        }
        ...
        if ($row) {
            ...
            if ($full) {
                ...
                $array['images'] = ($img) ? $img : $this->imagenesClass->checkForProduct($row['cod_producto']);
                $this->categoriasClass->set("cod", $row['categoria']);
                $array['category'] = $this->categoriasClass->view();
                $this->subcategoriasClass->set("cod", $row['subcategoria']);
                $array['subcategory'] = $this->subcategoriasClass->view();
                $array['link'] = URL . '/producto/' . $this->f->normalizar_link($row['titulo']) . '/' . $this->f->normalizar_link($row['cod']);
            }
        }
        return $array;
    }
    ...
    public function viewByCod($cod_producto)
    {
        ...
        if ($productos->num_rows != 0) {
            $row = !empty($row) ? $this->checkPriceByUser($row) : '';
            $img = $this->imagenesClass->listValidation($row['cod']);
            $img = ($img) ? $img : $this->imagenesClass->checkForProduct($row['cod_producto']);
            $this->categoriasClass->set("cod", $row['categoria']);
    ...
    function list($filter, $order, $limit, $full = true)
    {
        $array = [];
        if (is_array($filter)) {
            $filterSql = "WHERE ";
