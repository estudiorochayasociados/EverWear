...
if (isset($_GET["ordenImg"]) && isset($_GET["idImg"])) {
    ...
    $imagen->setOrder();
    $funciones->headerMove(URL_ADMIN . "/index.php?op=subcategorias&accion=modificar&cod=$cod");
}
...
if (isset($_POST["agregar"])) {
    ...
    $subcategoria->set("descripcion", $funciones->antihack_mysqli(isset($_POST["descripcion"]) ? $_POST["descripcion"] : ''));
    $subcategoria->set("orden", $funciones->antihack_mysqli(isset($_POST["orden"]) ? $_POST["orden"] : '0'));
...
<div class="col-md-12">
    ...
    <form method="post" class="row" enctype="multipart/form-data">
        ...
        <label class="col-md-3">
            Orden:<br />
            <input type="text" name="orden" value="<?= $dataSubcategoria["data"]["orden"] ?>" required>
        </label>
        <label class="col-md-3">
            Categoria:<br />
        ...
        <label class="col-md-12">
            Descripción:<br />
            <textarea type="text" class="ckeditorTextarea" name="descripcion" required><?= $dataSubcategoria["data"]["descripcion"] ?></textarea>
        </label>
        <div class="clearfix">
        </div>
        <?php
        foreach ($dataSubcategoria['images'] as $img) {
        ?>
            <div class='col-md-2 mb-20 mt-20'>
                ...
                <div class="row">
                    <div class="col-md-7">
                        <a href="<?= URL_ADMIN . '/index.php?op=subcategorias&accion=modificar&cod=' . $img['cod'] . '&borrarImg=' . $img['id'] ?>" class="btn btn-sm btn-block btn-danger">
                            BORRAR IMAGEN
                        </a>
                    </div>
                    <div class="col-md-5 text-right">
                        <select onchange='$(location).attr("href", "<?= CANONICAL ?>&idImg=<?= $img["id"] ?>&ordenImg="+$(this).val())'>
                            <?php
                            for ($i = 0; $i <= count($dataSubcategoria['images']); $i++) {
                                if ($img["orden"] == $i) {
